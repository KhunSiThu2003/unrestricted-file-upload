...
<div class="relative group">
    <div class="absolute -inset-1 bg-sky-600 rounded blur opacity-25 group-hover:opacity-40 transition duration-1000"></div>
    <div class="glass-panel relative rounded p-6 overflow-hidden backdrop-blur-xl">
            ...
            <div class="mb-6 info-badge rounded p-4">
                <div class="flex items-start gap-3">
                    <div class="w-6 h-6 rounded-full bg-sky-500/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                        <i class="fas fa-check-circle text-sky-400 text-sm"></i>
                    </div>
                    <p class="text-sm text-sky-300"><?php echo htmlspecialchars($success); ?></p>
                </div>
            </div>
        <?php endif; ?>
        ...
        <div id="panel-login" class="<?php echo $activeTab === 'login' ? '' : 'hidden'; ?> space-y-5">
            <h2 class="text-xl font-bold text-sky-300 mb-6">Sign in</h2>
            <form method="post" class="space-y-5">
                ...
                <p class="text-center text-sm text-gray-400 pt-2">
                    Don't have an account? <a href="#" class="tab-link text-sky-400 hover:text-sky-300 underline underline-offset-2" data-tab="register">Register</a>
                </p>
            </form>
